...
class VUAS_AchievementRenderer : EventHandler
{
    ...
    transient bool openBrowseMenuPending;

    ui transient int lastVuasTimeRelaySec;
    ui transient int lastToastSettingsRefreshGametic;
    ...
    {
        if (openBrowseMenuPending)
        {
            openBrowseMenuPending = false;
            Menu.SetMenu('VUASAchievementBrowse');
        }
    ...
    override void RenderOverlay(RenderEvent e)
    {
        ...
        // Relay SystemTime to play scope (BoA pattern). SystemTime has 1-second resolution.
        if (gametic >= 35)
        {
            int secNow = SystemTime.Now();
            if (secNow != lastVuasTimeRelaySec)
            {
                EventHandler.SendNetworkEvent("vuas_time", secNow);
                lastVuasTimeRelaySec = secNow;
            }
        }
        ...
        let task = taskQueue[0];
        int toastElapsed = task.GetElapsed(level.time);
        if (toastElapsed <= 0 || gametic - lastToastSettingsRefreshGametic >= 8)
        {
            rs.Refresh();
            lastToastSettingsRefreshGametic = gametic;
        }
    ...
    override void NetworkProcess(ConsoleEvent e)
    {
        ...
        if (e.Name ~== "vuas_open_browse_menu")
        {
            openBrowseMenuPending = true;
            return;
        }
