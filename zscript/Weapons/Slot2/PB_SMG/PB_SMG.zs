// PB_SMG - ZScript port (DECORATE PB_Weapon retired).
...
class PB_SMG : PB_WeaponBase
{
	...
	states
	{
		...
		A1F1 E 1 {
			...
			return PB_ReadyFire("Fire", "Fire", "Zoomout", false, false, "SMGAmmo", true, "UACSMGHasUnloaded");
		}
		Loop;
		...
		ReadyToFire2:
        A1F3 D 1 {
			return PB_ReadyFire("Fire2", "Fire2", "Zoomout", true, false, "SMGAmmo", true, "UACSMGHasUnloaded");
		}
		Loop;
		...
		A2F1 D 1 BRIGHT {
			A_AlertMonsters();
			A_PlaySoundEx("ZSpecOps/MGun", "SoundSlot5");
			A_FireBullets (0.1, 0.1, -1, 13, "HitPuff", FBF_NORANDOM, 8192, "Tracer", -4, 0);
			A_FireCustomMissile("YellowFlareSpawn",0,0,-4,0);
			A_PB_ThrottledGunSmoke(-4, 0, 0, 'UACSMGSmokePhase');
		...
		A2F1 A 1 BRIGHT {
			...
			A_FireBullets (0.1, 0.1, -1, 13, "HitPuff", FBF_NORANDOM,8192,"Tracer", -4, 8);
			A_FireCustomMissile("YellowFlareSpawn",0,0,4,0);
			A_PB_ThrottledGunSmoke(4, 0, 0, 'UACSMGSmokePhase');
		...
		TNT1 A 0 {
			if(CountInv("SelectLaserSight") == 1) {	
				...
				return ResolveState("ToggleLaser");
				}
			if(CountInv("SelectDualWieldSMG") == 1) {
				A_Overlay(-12, "WheelCancel");
				return ResolveState("DualHandler");
				}
			return ResolveState(null);
			}
			Goto Ready3;
		...
		TNT1 A 0 {
				...
				A_Print("\ctLaser sight:\c- \c-off");
				PB_HandleCrosshair(44);
				}
				else{
				A_GiveInventory("LaserSightActivated",1); 
				A_Print("\ctLaser sight:\c- \cdon");
				PB_HandleCrosshair(5);
				}
			}
		...
		TNT1 A 0 A_Print("\ckNeed two \ctsubmachine guns \c-to dual-wield");
		Goto Ready3;
		SwitchToDualWield:
		TNT1 A 0 {
		...
		"####" "#" 1 BRIGHT {
			...
			A_FireBullets (0.1, 0.1, -1, 13, "HitPuff", FBF_NORANDOM,8192,"Tracer", -2,0);
			A_FireCustomMissile("YellowFlareSpawn",0,0,0,0);
			A_PB_ThrottledGunSmoke(0, 0, 0, 'UACSMGSmokePhase');
			A_FireCustomMissile("ShakeYourAssMinor", 0, 0, 0, 0);
		...
		A1F3 E 1 BRIGHT {
			...
			A_FireBullets (0.1, 0.1, -1, 13, "HitPuff", FBF_NORANDOM,8192,"Tracer", -2,0);
			A_FireCustomMissile("YellowFlareSpawn",0,0,0,0);
			A_PB_ThrottledGunSmoke(0, 3, 0, 'UACSMGSmokePhase');
			A_FireCustomMissile("ShakeYourAssMinor", 0, 0, 0, 0);
...
//////////////////////////////////////////Unloading
		...
		PDA_Preview_SMG_Fire:
		"A1F1" F 1 BRIGHT;
		"A1F1" G 1 BRIGHT;
		"A1F1" H 1;
		Stop;
		PDA_Preview_SMG_AdsFire:
		"A1F3" E 1 BRIGHT;
		"A1F3" F 1;
		"A1F3" G 1;
		Stop;
		PDA_Preview_SMG_AltAds:
		"A1F3" A 1;
		"A1F3" B 1;
		"A1F3" C 1;
		Stop;
		PDA_Preview_SMG_Reload:
		"A1F4" A 1;
		"A1F4" D 1;
		"A1F4" J 1;
		"A1F4" K 1;
