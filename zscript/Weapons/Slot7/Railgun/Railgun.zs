...
class PB_Railgun : PB_WeaponBase
{
	default
	{
		...
		//Category Weapons
		//Sprite SRCGA0
	// SpawnID (DECORATE editor only) 9710
	...
	states
	{
		...
		ReadyToFire_Blue:
			R006 ABCDEFGHIJKLMNOPQRSTUVWXYZ 1 BRIGHT {
				...
				if (CountInv("RailgunAmmo") < 12){return ResolveState("ReadyToFire_Blue_Empty");}
				if (PressingAltfire()){return ResolveState("AltFire");}
				if (PressingFire() && CountInv("RailgunAmmo") > 0 && CountInv("RailgunCooldown") == 0){return ResolveState("Fire");}
		...
		ReadyToFire_Blue_Empty:
			R007 E 1 {
				if (PressingFire() && CountInv("RailgunAmmo") > 0){return ResolveState("Fire");}
				if (PressingAltfire()){return ResolveState("AltFire");}
		...
		ReadyToFire_Red:
			R012 ABCDEFGHIJKLMNOPQRSTUVWXYZ 1 BRIGHT {
				if (CountInv("RailgunAmmo") < 1){return ResolveState("ReadyToFire_Red_Empty");}
				if (PressingAltfire()){return ResolveState("AltFire");}
				if (PressingFire() && CountInv("RailgunAmmo") > 0){return ResolveState("Fire");}
				return A_DoPBWeaponAction(WRF_ALLOWRELOAD);
			}
		...
		ReadyToFire_Scoped:
			R028 F 1 BRIGHT {
				...
				if (CountInv("RailgunAmmo") == 0 ) { 
					return ResolveState("ReadyToFire_Scoped_Empty");
				}
				if(GetCvar("pb_toggle_aim_hold")) {
					...
					if (PressingFire() && PressingAltfire() && CountInv("RailgunAmmo" ) > 0 && CountInv("RailgunCooldown") == 0){
							return ResolveState("Fire_Scope");
					}
		...
		*/
		...
		AltFire:
			...
			TNT1 A 0 A_JumpIf(PressingReload(), "Railgun_DeployHologram");
		ZoomIn:
			TNT1 A 0 {
		...
		Laser_Stage_1:
			R036 BCDEF 1 BRIGHT {
				...
				if(CountInv("RailgunAmmo") < 1) {
					return ResolveState("Laser_Blast");
				}
			...
			R036 G 1 BRIGHT {
				...
				if(CountInv("RailgunLaserCharge") >= 15) {
					...
					return ResolveState("Laser_Stage_2");
				}
				return ResolveState("Laser_Stage_1"); //Return state fixes the laser mode charge where it stops charging if altfire isn't held down.;
			}
		Laser_Stage_2:
			R036 HIJKL 1 BRIGHT {
				PB_VisualLaser(CountInv("RailgunLaserCharge"));
			...
			R036 M 1 BRIGHT {
				...
				if(CountInv("RailgunLaserCharge") >= 30) {
					...
					return ResolveState("Laser_Stage_3");
				}
		...
		HoldLaser:
			...
			R036 TUVWXY 1 BRIGHT {
				...
				if(JustPressed(BT_RELOAD)) {
					return ResolveState("CancelLaserHold");
				}
				return ResolveState(null);
			}
			Loop;
		...
		Cancel_Stage1:
			...
			"####" T 1 BRIGHT {
				...
				A_SetBlend("Red",0.32,6);
				if(CountInv("RailgunLaserCharge") < 45) {
					return ResolveState("Cancel_Stage2");
				}
				return ResolveState("Cancel_Stage1");
			}
		Cancel_Stage2:
			"####" SRQPO 1 BRIGHT {
				if(CountInv("RailgunZoomed") >= 1) {
			...
			"####" N 1 BRIGHT {
				...
				if(CountInv("RailgunLaserCharge") < 30) {
					...
					return ResolveState("Cancel_Stage3");
				}
				return ResolveState("Cancel_Stage2");
			}
		Cancel_Stage3:
			...
			"####" H 1 BRIGHT {
				...
				A_SetBlend("Red",0.18,6);
				if(CountInv("RailgunLaserCharge") < 15) {
					return ResolveState("Cancel_Stage4");
				}
				return ResolveState("Cancel_Stage3");
			}
		Cancel_Stage4:
		...
		Laser_Blast:
			R036 Z 1 BRIGHT {
				...
				if(CountInv("RailgunLaserCharge") == 60) {
					...
					PB_FireLaser(CountInv("RailgunLaserCharge"));
				}
				else if(CountInv("RailgunLaserCharge") < 15) {
				...
				if (pb_weapon_recoil_extra_weapons) {
                    PB_WeaponRecoil(-0.25 * CountInv("RailgunLaserCharge"),-0.2 * CountInv("RailgunLaserCharge"));
                }
		...
		Laser_Stage_1_Scope:
			R039 BCDEF 1 BRIGHT {
				...
				if(JustReleased(BT_ATTACK) || !PressingFire()) {
					return ResolveState("Laser_Blast_Scope");
				}
				return ResolveState(null);
			}
			R039 G 1 BRIGHT {
				...
				if(CountInv("RailgunLaserCharge") >= 15) {
					A_StartSound("weapons/railgun/lasercharge1", CHAN_AUTO, CHANF_OVERLAP);
					return ResolveState("Laser_Stage_2_Scope");
				}
				return ResolveState("Laser_Stage_1_Scope"); //Return state fixes the laser mode charge where it stops charging if altfire isn't held down.;
			}
		Laser_Stage_2_Scope:
			R039 HIJKL 1 BRIGHT {
				PB_VisualLaser(CountInv("RailgunLaserCharge"));
			...
			R039 M 1 BRIGHT {
				...
				if(CountInv("RailgunLaserCharge") >= 30) {
					A_StartSound("weapons/railgun/lasercharge2", CHAN_AUTO, CHANF_OVERLAP);
					return ResolveState("Laser_Stage_3_Scope");
				}
				return ResolveState("Laser_Stage_2_Scope");
			}
		Laser_Stage_3_Scope:
		...
		Laser_Stage_4_Scope:
			R039 TUVWX 1 BRIGHT {
				...
				if(CountInv("RailgunLaserCharge") == 60) {
					A_StartSound("weapons/railgun/lasercharge4", CHAN_AUTO, CHANF_OVERLAP);
					return ResolveState("HoldLaser_Scope");
				}
				if(CountInv("RailgunAmmo") < 1) {
					return ResolveState("Laser_Blast_Scope");
				}
