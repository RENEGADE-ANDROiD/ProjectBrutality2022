Class PB_CSSG : PB_WeaponBase
{
	...
	Action state CSSG_HandleWheel()
	{
		...
		{
			A_Print("\coAlready using: \c-"..PB_CSSG.CSSG_ShellsType[actmode]);
		...
		XDeath:
			TNT1 A 0
			{
				...
				for (int i = 0; i < random(3,6); i++)
					A_SpawnItemEx("BigFlamesIG",frandom(-15,15),frandom(-15,15),frandom(-5,5),0,0,0,0,SXF_NOCHECKPOSITION);
	...
	{
		{
			...
			if (plr && plr.player && CVar.GetCVar("pb_lowgraphicsmode", plr.player).GetInt() != 0)
				return;
		}
		FSpawnParticleParams PFSMK;
		PFSMK.Texture = TexMan.CheckForTexture("X103E0");
