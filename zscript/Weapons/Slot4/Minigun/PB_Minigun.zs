// PB_Minigun - ZScript port (DECORATE PB_Weapon retired).

class PB_Minigun : PB_WeaponBase
{
	default
	{
		//Title Minigun
	...
	states
	{
		...
		MG14 ABCDEFGHIJKLMNOPQRSTUVWXYZ 1 {
			return A_DoPBWeaponAction();
			}
		MG15 ABC 1 {
			return A_DoPBWeaponAction();
		...
		Ready2:
			...
			CHAG ABCD 1 {
				if (CountInv("MinigunUpgraded") == 1) {return ResolveState("Ready2_Upgraded");}
				if (CountInv("NewClip") > 0 ) { A_Overlay(4,"AmmoBeltIdle"); }
				A_Overlay(3,"AmmoMeterOverlayIdle");
		...
		Ready2_Upgraded:
			...
			UCHG ABCDEFGH 1 {
				...
				A_Overlay(3,"AmmoMeterOverlayIdle");
				A_GunFlash("AltfireGlow");
				if (CountInv("TripleBarrelMode") == 1) {return ResolveState("Ready2_DeathDealer");}
				return A_DoPBWeaponAction();
			}
			TNT1 A 0 A_TakeInventory("Overheating", 1);
			Loop;
		...
		Hold:
		TNT1 A 0 {
			A_StartSound("weapons/minigunfire", CHAN_WEAPON, CHANF_LOOP|CHANF_NOSTOP);
			A_ZoomFactor(0.980);
        ...
        CHAF A 1 BRIGHT {
			...
			A_FireBullets(4, 4, 1, 11, "MachineGunBulletPuff", 1);
			A_PB_ThrottledGunSmoke(0, 0, 0, 'MinigunSmokePhase');
        ...
        TNT1 A 0 A_ReFire("Holding1CG_Loop");
		Holding1CG_EndFire:
		TNT1 A 0 A_JumpIfInventory("Spin",1,"Ready2");
		TNT1 A 0 {
			A_ClearOverlays(4,4);
        ...
        TNT1 A 0 {
			A_StartSound("weapons/minigunfirefast", CHAN_WEAPON, CHANF_LOOP|CHANF_NOSTOP);
			if (CountInv("MinigunUpgraded") == 1) {A_Overlay(4,"AmmoBeltAnimation_Upgraded");}
			else {A_Overlay(4,"AmmoBeltAnimation");}
			}
		...
		FiringFast:
		TNT1 A 0 A_GunFlash("PostGlow");
		Goto FiringFast_Loop;
		FiringFast_Loop:
		CHAF A 1 BRIGHT {
			if (CountInv("MinigunUpgraded") == 1) {A_SetWeaponSprite("UCHF");}
			A_Overlay(3,"AmmoMeterOverlayFire");
			A_FireBullets(4, 3, -1, 11, "MachineGunBulletPuff", 1);
			A_PB_ThrottledGunSmoke(0, 0, 0, 'MinigunSmokePhase');
			if (GetCvar("PB_alttracer") >= 1) {
				A_RailAttack(0, 0, 0, -1, -1, RGF_SILENT|RGF_NOPIERCING|RGF_EXPLICITANGLE, 0, 'none', 0.1, 0.1, 700, 0, 1, 1.0, "Tracer_Trail", -6, 0, 0);
			} else {
		...
		"8HBF" A 1 BRIGHT {
			A_PB_ThrottledGunSmoke(0, 0, 0, 'MinigunSmokePhase');
			if (GetCvar("PB_alttracer") >= 1) {
				A_RailAttack(0, -4, 0, -1, -1, RGF_SILENT|RGF_NOPIERCING|RGF_EXPLICITANGLE, 0, 'none', 0.1, 0.1, 700, 0, 1, 1.0, "Tracer_Trail", -6, 0, 0);
				A_RailAttack(0, 0, 0, -1, -1, RGF_SILENT|RGF_NOPIERCING|RGF_EXPLICITANGLE, 0, 'none', 0.1, 0.1, 700, 0, 1, 1.0, "Tracer_Trail", -6, 0, 0);
				A_RailAttack(0, 4, 0, -1, -1, RGF_SILENT|RGF_NOPIERCING|RGF_EXPLICITANGLE, 0, 'none', 0.1, 0.1, 700, 0, 1, 1.0, "Tracer_Trail", -6, 0, 0);
			} else {
				A_FireCustomMissile("MinigunTracer", random(-6, 6), 0, -1, random(-6, 6));
			...
			A_Firecustommissile("FiftyCaseSpawn",0,0,-12,-18);
			A_Firecustommissile("MiniBeltPieceSpawn",0,0,-12,-18);
			A_FireBullets(7, 7, 1, 10, "MachineGunBulletPuff", 1);
			A_FireBullets(7, 7, 1, 10, "MachineGunBulletPuff", 1);
			A_FireBullets(7, 7, 0, 10, "MachineGunBulletPuff", 1);
			A_Overlay(3,"AmmoMeterOverlayIdle_DeathDealer");
			;//A_GunFlash();
            PB_WeaponRecoil(-0.8,frandom(2.4, -2.4));
			}
		...
		PDA_Preview_MiniReady:
			"MG10" A 1 A_WeaponReady(WRF_NOFIRE);
			"MG10" B 1 A_WeaponReady(WRF_NOFIRE);
			Stop;
		PDA_Preview_MiniSpin:
			"CHAG" A 1 A_WeaponReady(WRF_NOFIRE);
			"CHAG" B 1 A_WeaponReady(WRF_NOFIRE);
			"CHAG" C 1 A_WeaponReady(WRF_NOFIRE);
			"CHAG" D 1 A_WeaponReady(WRF_NOFIRE);
			Stop;
		PDA_Preview_MiniBurst:
			"CHAF" A 1 BRIGHT A_WeaponReady(WRF_NOFIRE);
			"CHAF" B 1 BRIGHT A_WeaponReady(WRF_NOFIRE);
			"CHAF" C 1 BRIGHT A_WeaponReady(WRF_NOFIRE);
			Stop;
		PDA_Preview_MiniAltSpin:
			"CHAG" A 1 A_WeaponReady(WRF_NOFIRE);
			"CHAG" B 1 A_WeaponReady(WRF_NOFIRE);
			"CHAG" C 1 A_WeaponReady(WRF_NOFIRE);
			"CHAG" D 1 A_WeaponReady(WRF_NOFIRE);
