// PB_MG42 - ZScript port (DECORATE PB_Weapon retired).

class PB_MG42 : PB_WeaponBase
{
	default
	{
		//Title MG-42
		//Category Weapons
		//Sprite HBUSD0
		// Game Doom (DECORATE only)
		// SpawnID (DECORATE editor only)
	Weapon.BobRangeX 0.3;
	Weapon.BobRangeY 0.5;
		Weapon.BobStyle "InverseSmooth";
	Weapon.BobSpeed 2.4;
	+FLOORCLIP;
	Weapon.AmmoType1 "NewClip";
	Weapon.AmmoType2 "MG42HeatLevel";
	Weapon.AmmoGive1 30;
	Weapon.AmmoGive2 0;
	Weapon.AmmoUse1 0;
	Inventory.PickupMessage "You got MG-42 aka Hitler's Buzzsaw! (Slot 5)";
	Weapon.AmmoUse2 0;
	Inventory.PickupSound "weapons/MG42/Pickup";
	Obituary "%o tasted the power of %k's MG42";
	+WEAPON.NOAUTOAIM;
	+WEAPON.AMMO_OPTIONAL;
	+WEAPON.NOAUTOFIRE;
	+WEAPON.NOALERT;
	Scale 0.28;
	Tag "MG-42 (Hitler's Buzzsaw)";
	Inventory.Icon "HBUSD0";
	Inventory.AltHUDIcon "HBUSD0";
	PB_WeaponBase.respectItem "MG42Respected";
	//FloatBobStrength 0.5
	Weapon.SlotNumber 5;
	Weapon.SlotPriority 0.05;
	}
	states
	{
	
	//Short stuff first
		WeaponSpecial:
		TNT1 A 0 A_Print("\ctWeapon Special:\c- \cdutility \c-");
		TNT1 A 0 A_Takeinventory("GoWeaponSpecialAbility",1);
		Goto Ready3;
		EmptyFire:
		TNT1 A 0 A_PlaySoundEx("weapons/empty", "Auto");
		Goto Ready3;
		Steady:
		TNT1 A 1;
		Goto Ready;
		Cooling:
		TNT1 A 4 A_TakeInventory("MG42HeatLevel", 1);
		TNT1 A 0 {
			if(CountInv("MG42HeatLevel") <= 60) {A_TakeInventory("MG42HasOverheated");}
			}
		Loop;
		Spawn:
		"HBUS" D 1;
		Loop;

		Select:
		TNT1 A 0 {
			A_WeaponOffset(0,32);
			A_SetRoll(0);
			PB_HandleCrosshair(50);
			A_TakeInventory("PB_LockScreenTilt",1);
		}
		Goto SelectFirstPersonLegs;
		SelectContinue:
		TNT1 A 0;
		TNT1 A 0 A_ZoomFactor(1.0);
		TNT1 A 0 PB_WeapTokenSwitch("MG42Selected");
 		TNT1 A 0 A_Giveinventory("RandomHeadExploder",1);
		TNT1 AAAAAAAAAAAAAAAAAA 0 A_Raise;
		TNT1 AAAAAAAA 1 A_Raise;
		Wait;
	
		BarrelSmoke1:
		TNT1 A 0 A_JumpIfInventory("MG42HeatLevel", 35, "SmokeReal");
		Stop;
		SmokeReal:
		TNT1 A 0 {
			If(CountInv("MG42HeatLevel") >=85 || CountInv("MG42HeatLevel") ==100) {
				return ResolveState("OverheatedSmoke");
				}
				return ResolveState(null);
			}
		ActualSmoke:
		TNT1 AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA 1 {
			A_FireCustomMissile("GunBarrelSmoke", 0, 0, 0, 0, 0, 0);
			A_FireCustomMissile("GunBarrelSmoke", 0, 0, 0, 0, 0, 0);
		}
	Stop;
		OverheatedSmoke:
		TNT1 AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA 1 {
			A_FireCustomMissile("GunFireSmoke_Overheat", 0, 0, 0, 1, 0, 0);
			A_FireCustomMissile("GunFireSmoke_Overheat", 0, 0, 0, -1, 0, 0);
			}
		Goto ActualSmoke;
	
		WeaponRespect:
		TNT1 A 0 {
		A_SetCrosshair(5);
		A_GiveInventory("MG42Respected", 1);
		A_PlaySound("weapons/MG42/Select");
		A_TakeInventory("MG42HeatLevel",100); // If give all or give everything is used;
		}
		"MGR1" ABCDEFGHIJKLMNOPQRST 1 A_DoPBWeaponAction;
		TNT1 A 0 A_PlaySound("MG42OP");
		"MGR1" UVWXYZ 1 A_DoPBWeaponAction;
		"MGR2" ABCDEFGHIJK 1 A_DoPBWeaponAction;
		TNT1 A 0 A_PlaySound("MG42RE");
		"MGR2" LMNOPQRSRSTUVWXYZ 1 A_DoPBWeaponAction;
		TNT1 A 0 A_PlaySound("weapons/smg_magfly1");
		"MGR3" ABCDEFGHIJKLMNOPQRST 1 A_DoPBWeaponAction;
		TNT1 A 0 A_PlaySound("MG42BUL");
		"MGR3" UVWXYZ 1 A_DoPBWeaponAction;
		"MGR4" ABCDE 1 A_DoPBWeaponAction;
		TNT1 A 0 A_PlaySound("MG42BUL");
		"MGR4" FGHIJKLMNOPQRSTUV 1 A_DoPBWeaponAction;
		TNT1 A 0 A_PlaySound("weapons/MG42/Pickup");
		"MGR4" WXYZ 1 A_DoPBWeaponAction;
		"MGR5" ABCDEF 1 A_DoPBWeaponAction;
		TNT1 A 0 A_PlaySound("MG42IN");
		"MGR5" GHIJKLMNOP 1 A_DoPBWeaponAction;
		TNT1 A 0 A_PlaySoundEx("weapons/riflemagslap", "Auto");
		"MGR5" QRSTUVWXYZ 1 A_DoPBWeaponAction;
		"MGR6" ABCDEFHI 1 A_DoPBWeaponAction;
		TNT1 A 0 A_PlaySound("MG42CL");
		"MGR6" JKLMNOPQRST 1 A_DoPBWeaponAction;
		"MGRC" EFGH 1 A_DoPBWeaponAction;
		TNT1 A 0 A_PlaySound("MG42RC");
		"MGRC" HHHHGFE 1 A_DoPBWeaponAction;
		"MGR6" UVWX 1 A_DoPBWeaponAction;
		TNT1 A 0 PB_HandleCrosshair(50);
		Goto Ready3;
		
		
		SelectBelt:
		"MGS5" A 0 A_JumpIfInventory("NewClip", 14, 5);
		"MGS4" A 0 A_JumpIfInventory("NewClip", 12, 4);
		"MGS3" A 0 A_JumpIfInventory("NewClip", 10, 3);
		"MGKS" A 0 A_JumpIfInventory("NewClip", 8, 2);
		TNT1 A 0;
		"####" A 0;
		"####" CBA 1;
		Stop;
		
		Ready:
		TNT1 A 0 A_JumpIfInventory("GoFatality", 1, "Steady");
		TNT1 A 1 A_WeaponReady(WRF_NOFIRE);
		TNT1 A 0 A_Overlay(3, "Cooling");
		TNT1 A 0 A_TakeInventory("MG42HeatLevel", 20);
		TNT1 A 0 PB_RespectIfNeeded;
		SelectAnimation:
		TNT1 A 0 {
		A_PlaySound("weapons/MG42/Select");
		if(CountInv("MG42HeatLevel") > 1) {A_Overlay(3, "Cooling");}
		}
		"MGSE" FE 1;
		TNT1 A 0 {
		A_GunFlash("SelectBelt");
		}
		"MGSE" DCB 1;

		Ready3:
		"MG5R" A 0;
		"MG4R" A 0;
		"MG3R" A 0;
		"MG2R" A 0;
		"MG1R" A 0;
		"####" A 0 {
			if(CountInv("MG42HeatLevel") > 1) {A_Overlay(3, "Cooling");}
		}
		ReadyToFire:
		MG1R A 1 {
			if(CountInv("NewClip") >= 14) {A_SetWeaponSprite("MG5R");}
			if(CountInv("NewClip") >= 12 && CountInv("NewClip") < 14) {A_SetWeaponSprite("MG4R");}
			if(CountInv("NewClip") >= 10 && CountInv("NewClip") < 12) {A_SetWeaponSprite("MG3R");}
			if(CountInv("NewClip") >= 8 && CountInv("NewClip") < 10) {A_SetWeaponSprite("MG2R");
			}
			return A_DoPBWeaponAction(WRF_ALLOWRELOAD);
		}
		Loop;
	
		ChangeBullet:
		"MA15" A 0 A_JumpIfInventory("NewClip", 14, 5);
		"MA14" A 0 A_JumpIfInventory("NewClip", 12, 4);
		"MA13" A 0 A_JumpIfInventory("NewClip", 10, 3);
		"MA12" A 0 A_JumpIfInventory("NewClip", 8, 2);
		TNT1 A 0;
		"####" ABCDEFGHIIIJKLMNOPQRSTTTTTTUVVVVWXYZ 1;
		"MA25" A 0 A_JumpIfInventory("NewClip", 14, 5);
		"MA24" A 0 A_JumpIfInventory("NewClip", 12, 4);
		"MA23" A 0 A_JumpIfInventory("NewClip", 10, 3);
		"MA22" A 0 A_JumpIfInventory("NewClip", 8, 2);
		TNT1 A 0;
		"####" AAAAAABCDEFFFFFFGHHHHIJKLMNNNN 1;
		//RECHAMBER
		"MG5C" A 0 A_JumpIfInventory("NewClip", 14, 5);
		"MG4C" A 0 A_JumpIfInventory("NewClip", 12, 4);
		"MG3C" A 0 A_JumpIfInventory("NewClip", 10, 3);
		"MG2C" A 0 A_JumpIfInventory("NewClip", 8, 2);
		TNT1 A 0;
		"####" ABCDDDDCBA 1;
		"MA25" A 0 A_JumpIfInventory("NewClip", 14, 5);
		"MA24" A 0 A_JumpIfInventory("NewClip", 12, 4);
		"MA23" A 0 A_JumpIfInventory("NewClip", 10, 3);
		"MA22" A 0 A_JumpIfInventory("NewClip", 8, 2);
		TNT1 A 0;
		"####" NNOPQRST 1;
		Stop;
		Reload:
		TNT1 A 0 A_JumpIf(CountInv("MG42HasOverheated") || CountInv("MG42BarrelOverheat") >= 1, "BarrelChange");
		Goto ReadyToFire;
		UnzoomOverheat:
		TNT1 A 0 {
		A_TakeInventory("Zoomed", 1);
		A_TakeInventory("ADSMode", 1);
		A_PlaySoundEx("IronSights", "Auto");
		A_Overlay(5, "BeltUnzoomFlash");
		A_ZoomFactor(1);
		A_PlaySound("MG42HEAT");
		PB_HandleCrosshair(50);
		}
		"MRGZ" DCB 1;
		Goto Overheat+1;
		UnzoomBarrelChange:
	TNT1 A 0 {
		A_TakeInventory("Zoomed", 1);
		A_TakeInventory("ADSMode", 1);
		A_PlaySoundEx("IronSights", "Auto");
		A_Overlay(5, "BeltUnzoomFlash");
		A_ZoomFactor(1);
		PB_HandleCrosshair(50);
		}
	"MRGZ" DCB 1;
	Goto BarrelChange+2;
		BarrelChange:
		TNT1 A 0 A_JumpIfInventory("Zoomed",1,"UnzoomBarrelChange");
		TNT1 AA 0;
		TNT1 A 0{
		A_ClearOverlays(5, 6);
		A_TakeInventory("MG42HeatLevel", 200);
		A_TakeInventory("MG42HasOverheated");
		A_TakeInventory("MG42BarrelOverheat");
		A_Overlay(3, "ChangeBullet");
		}
		"MGC1" ABCDEFGHIJKLMNO 1;
		TNT1 A 0 A_PlaySound("MG42OP");
		"MGC1" PQRSTUVWXYZ 1;
		"MGC2" ABCDEF 1;
		TNT1 A 0 A_PlaySound("MG42RE");
		"MGC2" GHIJKLMNOP 1;
		TNT1 A 0 A_PlaySound("MG42IN");
		"MGC2" QRSTUVWXYZ 1;
		"MGC3" ABCDEFG 1;
		TNT1 A 0 A_PlaySound("MG42CL");
		"MGC3" HIJKLMN 1;
		"MGRC" ABC 1;
		TNT1 A 0 A_PlaySound("MG42RC");
		"MGRC" DDDDCBA 1;
		"MGC3" OPQRST 1;
		TNT1 A 0 A_Overlay(3, "Cooling");
		Goto Ready3;
		Fire:
		TNT1 A 0 A_JumpIfInventory("MG42HasOverheated",1,"BarrelChange");
		TNT1 A 0 A_JumpIfInventory("Zoomed", 1, "Fire2");
		TNT1 A 0 {
		if (CountInv("NewClip") >= 1 && CountInv("MG42HeatLevel") != 100) {
			return ResolveState("ActualFire");
			}
		else if (CountInv("MG42HeatLevel") == 100) {
			return ResolveState("Overheat");
			}
		else {
			return ResolveState(null);
			}
		}
		Goto EmptyFire;
	
		MuzzleFlash:
		TNT1 A 0 A_Jump(128, "Flash1", "Flash2", "Flash3");
		Flash3:
		"MGMZ" EF 1 BRIGHT;
		Stop;
		Flash2:
		"MGMZ" CD 1 BRIGHT;
		Stop;
		Flash1:
		"MGMZ" AB 1 BRIGHT;
		Stop;
	
		BeltFlash:
		"MGF5" A 0 A_JumpIfInventory("NewClip", 14, 5);
		"MGF4" A 0 A_JumpIfInventory("NewClip", 12, 4);
		"MGF3" A 0 A_JumpIfInventory("NewClip", 10, 3);
		"MGF2" A 0 A_JumpIfInventory("NewClip", 8, 2);
		TNT1 A 0;
		"####" ABCD 1;
		Stop;
		ActualFire:
		TNT1 A 0 {
		A_ClearOverlays(3, 6);
		A_Overlay(-3, "MuzzleFlash");
		A_Overlay(5, "BeltFlash");
		if(CountInv("MG42BarrelOverheat") >= 1) {
			A_GiveInventory("MG42HeatLevel",3);
			}
		else{
			A_GiveInventory("MG42HeatLevel", 1);
			}
		A_TakeInventory("NewClip", 1);
		A_PlaySound("MG42FIR", 1);
		A_FireBullets(5, 6, 1, 31, "MachineGunBulletPuff", 1);
		A_PB_MG42FireFX(1);
		A_FireCustomMissile("ShakeYourAss", 0, 0, 0, 0);
		A_AlertMonsters();
		PB_WeaponRecoil(-2.2,-0.6);
		}
		"MGFI" AB 1;
		TNT1 A 0 A_Refire();
		TNT1 A 0 A_Overlay(6, "BarrelSmoke1");
		"MGFI" CD 1;
		TNT1 A 0 A_Overlay(3, "Cooling");
		Goto Ready3;
		Overheat:
		TNT1 A 0 A_PlaySound("MG42HEAT");
		TNT1 A 0 A_GiveInventory("MG42HasOverheated");
		TNT1 A 0 A_GiveInventory("MG42BarrelOverheat");
		"MG5R" A 0 A_JumpIfInventory("NewClip", 14, 5);
		"MG4R" A 0 A_JumpIfInventory("NewClip", 12, 4);
		"MG3R" A 0 A_JumpIfInventory("NewClip", 10, 3);
		"MG2R" A 0 A_JumpIfInventory("NewClip", 8, 2);
		"MG1R" A 0;
		"####" A 0;
		"####" A 0 A_Overlay(3,"Cooling");
		"####" A 0 A_Overlay(6, "BarrelSmoke1");
		"####" A 45 A_DoPBWeaponAction(WRF_NOFIRE|WRF_NOSWITCH);
		Goto ReadyToFire;
	
		BeltZoomFlash:
		"MGZ5" A 0 A_JumpIfInventory("NewClip", 14, 5);
		"MGZ4" A 0 A_JumpIfInventory("NewClip", 12, 4);
		"MGZ3" A 0 A_JumpIfInventory("NewClip", 10, 3);
		"MGZ2" A 0 A_JumpIfInventory("NewClip", 8, 2);
		TNT1 A 0;
		"####" A 0;
		"####" AB 1;
		"MGZ3" A 0 A_JumpIfInventory("NewClip", 10, 3);
		"MGZ2" A 0 A_JumpIfInventory("NewClip", 8, 2);
		TNT1 A 0;
		"####" A 0;
		"####" C 1;
		Stop;
	
		AltFire:
		TNT1 A 0 A_JumpIfInventory("Zoomed", 1, "Unzoom");
		TNT1 A 0 {
		A_GiveInventory("Zoomed", 1);
		A_GiveInventory("ADSMode", 1);
		A_PlaySoundEx("IronSights", "Auto");
		A_Overlay(5, "BeltZoomFlash");
		A_ZoomFactor(1.25);
		A_SetCrosshair(5);
		}
		"MRGZ" BCD 1;
		Goto Ready2;
	
		BeltUnzoomFlash:
		"MGZ3" A 0 A_JumpIfInventory("NewClip", 10, 3);
		"MGZ2" A 0 A_JumpIfInventory("NewClip", 8, 2);
		TNT1 A 0;
		"####" A 0;
		"####" C 1;
		"####" A 0;
		"MGZ5" A 0 A_JumpIfInventory("NewClip", 14, 5);
		"MGZ4" A 0 A_JumpIfInventory("NewClip", 12, 4);
		"MGZ3" A 0 A_JumpIfInventory("NewClip", 10, 3);
		"MGZ2" A 0 A_JumpIfInventory("NewClip", 8, 2);
		TNT1 A 0;
		"####" A 0;
		"####" BA 1;
		Stop;
	
		Unzoom:
		TNT1 A 0 {
		A_TakeInventory("Zoomed", 1);
		A_TakeInventory("ADSMode", 1);
		A_PlaySoundEx("IronSights", "Auto");
		A_Overlay(5, "BeltUnzoomFlash");
		A_ZoomFactor(1);
		PB_HandleCrosshair(50);
		}
		"MRGZ" DCB 1;
		Goto Ready3;
	
		Ready2:
		"MG3Z" A 0 A_JumpIfInventory("NewClip", 10, 3);
		"MG2Z" A 0 A_JumpIfInventory("NewClip", 8, 2);
		"MG1Z" A 0;
		"####" A 0;
		"####" A 1 {
		//Some ADS toggle stuff
		//I am sorry for YandereDev shit... i didn't code for a long time lol
		A_SetCrosshair(5);
		if(GetCvar("pb_toggle_aim_hold")) {
				if(!PressingAltfire() || JustReleased(BT_ALTATTACK) ) {
					return ResolveState("Unzoom");
				}
				if (PressingFire() && PressingAltfire() && CountInv("NewClip") > 1 && CountInv("MG42HeatLevel") < 100) {
							return ResolveState("Fire2");
							}
					else if (PressingFire() && PressingAltfire() && CountInv("NewClip") > 1 && CountInv("MG42HeatLevel") == 100) {
							return ResolveState("UnzoomOverheat");
						}
					else if (PressingFire() && PressingAltfire() && CountInv("NewClip") <= 1) {
							return ResolveState("Unzoom");
						}
				return A_DoPBWeaponAction(WRF_ALLOWRELOAD|WRF_NOSECONDARY);
			}
		else {
			return A_DoPBWeaponAction();
		}
	}
	Loop;
	
		BeltFlashZoomFire:
		"MZF3" A 0 A_JumpIfInventory("NewClip", 10, 3);
		"MZF2" A 0 A_JumpIfInventory("NewClip", 8, 2);
		TNT1 A 0;
		"####" A 0;
		"####" AC 1;
		Stop;
		BeltFlashZoomFireFinished:
		TNT1 B 1{
			if(CountInv("NewClip") >= 10) {A_SetOverlaySprite(5, "MZF3");}
			else if (CountInv("NewClip")  == 8 || CountInv("NewClip")  == 9)  {A_SetOverlaySprite(5, "MZF2");}
		}
		STOP;
		
		MuzzleFlashZoom:
		TNT1 A 0 A_Jump(128, "MuzzleZoom1", "MuzzleZoom2", "MuzzleZoom3");
		MuzzleZoom1:
		"MGZM" A 1 BRIGHT;
		Stop;
		MuzzleZoom2:
		"MGZM" B 2 BRIGHT;
		Stop;
		MuzzleZoom3:
		"MGZM" C 1 BRIGHT;
		Stop;
		Fire2:
		TNT1 A 0 A_JumpIfInventory("MG42HasOverheated",1,"UnzoomBarrelChange");
		TNT1 A 0 {
		if (CountInv("NewClip") >= 1 && CountInv("MG42HeatLevel") != 100) {
			return ResolveState("ActualFire2");
			}
		else if (CountInv("MG42HeatLevel") == 100) {
			return ResolveState("UnzoomOverheat");
			}
		else {
			return ResolveState("Unzoom");
			}	
		}	
		Goto Ready2;
		ActualFire2:
		TNT1 A 0 {
		A_ClearOverlays(3, 4);
		A_Overlay(-3, "MuzzleFlashZoom");
		A_Overlay(5, "BeltFlashZoomFire");
		if(CountInv("MG42BarrelOverheat") >= 1) {
			A_GiveInventory("MG42HeatLevel",4);
			}
		else{
			A_GiveInventory("MG42HeatLevel", 2);
			}
		A_TakeInventory("NewClip", 1);
		A_PlaySound("MG42FIR", 1);
		A_FireBullets(5, 5, 1, 31, "MachineGunBulletPuff", 1);
		A_PB_MG42FireFX(0);
		A_FireCustomMissile("ShakeYourAss", 0, 0, 0, 0);
		A_AlertMonsters();
		PB_WeaponRecoil(-2.0,-0.5);
		}
		"MGZF" AB 1;
		TNT1 A 0 {
		if (CountInv("MG42HeatLevel") == 100) {
			return ResolveState("UnzoomOverheat");
			}
		else {
			A_Overlay(3, "Cooling");
			return ResolveState("FinishedZoomFire");
			}	
		}
		FinishedZoomFire:
		TNT1 A 0 {		
			if(GetCvar("pb_toggle_aim_hold")) {
				if(JustReleased(BT_ALTATTACK)){
					return ResolveState("UnZoom");
				}
		
				if (PressingFire() && PressingAltfire()){
						return ResolveState("Fire2");
				}
			}
			else {
				if(PressingAltfire()){
					return ResolveState("UnZoom");
				}
				A_Refire("Fire2");
			}
			return ResolveState(null);
		}
		"MGZF" C 1 A_Overlay(5, "BeltFlashZoomFireFinished");
		MG1Z A 1 {
			if(CountInv("NewClip") >= 10) {A_SetWeaponSprite("MG3Z");}
			if(CountInv("NewClip") == 8 || CountInv("NewClip") == 9) {A_SetWeaponSprite("MG2Z");}
			}
		TNT1 A 0 A_Overlay(3, "Cooling");
		Goto Ready2;
	
		FlashKicking:
		"MGK5" A 0 A_JumpIfInventory("NewClip", 14, 5);
		"MGK4" A 0 A_JumpIfInventory("NewClip", 12, 4);
		"MGK3" A 0 A_JumpIfInventory("NewClip", 10, 3);
		"MGK2" A 0 A_JumpIfInventory("NewClip", 8, 2);
		"MGKI" A 0;
		"####" A 0;
		"####" BCD 1;
		"MGKI" EFGHHGFE 1;
		"MGK5" A 0 A_JumpIfInventory("NewClip", 14, 5);
		"MGK4" A 0 A_JumpIfInventory("NewClip", 12, 4);
		"MGK3" A 0 A_JumpIfInventory("NewClip", 10, 3);
		"MGK2" A 0 A_JumpIfInventory("NewClip", 8, 2);
		"MGKI" A 0;
		"####" A 0;
		"####" DCB 1;
		Goto Ready3;
		FlashAirKicking:
		"MGK5" A 0 A_JumpIfInventory("NewClip", 14, 5);
		"MGK4" A 0 A_JumpIfInventory("NewClip", 12, 4);
		"MGK3" A 0 A_JumpIfInventory("NewClip", 10, 3);
		"MGK2" A 0 A_JumpIfInventory("NewClip", 8, 2);
		"MGKI" A 0;
		"####" A 0;
		"####" BCD 1;
		"MGKI" EFGHIIIHGFE 1;
		"MGK5" A 0 A_JumpIfInventory("NewClip", 14, 5);
		"MGK4" A 0 A_JumpIfInventory("NewClip", 12, 4);
		"MGK3" A 0 A_JumpIfInventory("NewClip", 10, 3);
		"MGK2" A 0 A_JumpIfInventory("NewClip", 8, 2);
		"MGKI" A 0;
		"####" A 0;
		"####" DCB 1;
		Goto Ready3;
		FlashSlideKicking:
		"MGK5" A 0 A_JumpIfInventory("NewClip", 14, 5);
		"MGK4" A 0 A_JumpIfInventory("NewClip", 12, 4);
		"MGK3" A 0 A_JumpIfInventory("NewClip", 10, 3);
		"MGK2" A 0 A_JumpIfInventory("NewClip", 8, 2);
		"MGKI" A 0;
		"####" A 0;
		"####" BCD 1;
		"MGKI" EFGHIIIIIIIIIIHGFE 1;
		"MGK5" A 0 A_JumpIfInventory("NewClip", 14, 5);
		"MGK4" A 0 A_JumpIfInventory("NewClip", 12, 4);
		"MGK3" A 0 A_JumpIfInventory("NewClip", 10, 3);
		"MGK2" A 0 A_JumpIfInventory("NewClip", 8, 2);
		"MGKI" A 0;
		"####" A 0;
		"####" DCB 1;
		Goto Ready3;
		FlashSlideKickingStop:
		"MGKI" IIII 1;
		"MGK5" A 0 A_JumpIfInventory("NewClip", 14, 5);
		"MGK4" A 0 A_JumpIfInventory("NewClip", 12, 4);
		"MGK3" A 0 A_JumpIfInventory("NewClip", 10, 3);
		"MGK2" A 0 A_JumpIfInventory("NewClip", 8, 2);
		"MGKI" A 0;
		"####" A 0;
		"####" DCB 1;
		Goto Ready3;
		PDA_Preview_MG42_Fire:
		"MGFI" AB 2;
		"MGFI" CD 2;
		Stop;
		PDA_Preview_MG42_Reload:
		"MGC2" ABCDEFGHIJKLMNOP 2;
		"MGC2" QRSTUVWXYZ 2;
		"MGC3" ABCDEFG 2;
		"MGRC" ABC 2;
		Stop;

		FlashPunching:
		"MGK5" A 0 A_JumpIfInventory("NewClip", 14, 5);
		"MGK4" A 0 A_JumpIfInventory("NewClip", 12, 4);
		"MGK3" A 0 A_JumpIfInventory("NewClip", 10, 3);
		"MGK2" A 0 A_JumpIfInventory("NewClip", 8, 2);
		"MGKI" A 0;
		"####" A 0;
		"####" BCD 1;
		"MGKI" EFGHHGFE 1;
		"MGK5" A 0 A_JumpIfInventory("NewClip", 14, 5);
		"MGK4" A 0 A_JumpIfInventory("NewClip", 12, 4);
		"MGK3" A 0 A_JumpIfInventory("NewClip", 10, 3);
		"MGK2" A 0 A_JumpIfInventory("NewClip", 8, 2);
		"MGKI" A 0;
		"####" A 0;
		"####" DCB 1;
		TNT1 A 0 A_ClearOverlays(PSP_FLASH, PSP_FLASH, false);
		Goto Ready3;
	
		DeselectFlash:
		"MGS5" A 0 A_JumpIfInventory("NewClip", 14, 5);
		"MGS4" A 0 A_JumpIfInventory("NewClip", 12, 4);
		"MGS3" A 0 A_JumpIfInventory("NewClip", 10, 3);
		"MGKS" A 0 A_JumpIfInventory("NewClip", 8, 2);
		TNT1 A 0;
		"####" A 0;
		"####" ABC 1;
		Stop;
	
		Deselect:
		TNT1 A 0 {
			A_WeaponOffset(0,32);
			A_SetRoll(0);
			PB_HandleCrosshair(50);
			A_TakeInventory("PB_LockScreenTilt",1);
			A_ClearOverlays(10,11);
		}
		TNT1 A 0 {
			A_GunFlash("DeselectFlash");
			A_Takeinventory("Zoomed",1);
			A_Takeinventory("ADSmode",1);
			A_ZoomFactor(1.0);
			A_PlaySoundEx("weapons/changing", "Auto");
			}
		"MGSE" BCDEF 1;
		TNT1 AAAAAAAAAAAAAAAAAA 0 A_Lower;
		Wait;
	}
}
