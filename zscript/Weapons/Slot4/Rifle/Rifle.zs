// Rifle - ZScript port (DECORATE PB_Weapon retired).

class Rifle : PB_WeaponBase
{
	default
	{
		//Title Rifle
	...
	states
	{
		...
		InspectUpgrade:
			...
			TNT1 A 0 A_PlaySoundEx("Weapons/GrenadeOpen", "Auto");
			"R6F1" "LMNOP" 1 A_DoPBWeaponAction();
			TNT1 A 0  A_PlaySoundEx("Weapons/GrenadeLoad", "Auto");
			"R6F1" "QRSTUVWXYZ" 1 A_DoPBWeaponAction();
			"R6F2" A 1 A_PlaySoundEx("LMGCLK", "Auto");
			"R6F2" "BCDEFG" 1;
			TNT1 A 0 A_PlaySoundEx("IronSights", "Auto");
		...
		Ready3:
			...
			TNT1 A 0 A_JumpIf(PressingAltfire() && CountInv("HDMRGrenadeMode") == 1, "PrepareRifleGrenadeFire");
...
//Overlays for complex reticle.  Will add more for the sniper mode later
			...
			H4F6 G 1 BRIGHT {
				...
				if(GetCvar("pb_toggle_aim_hold")) {
					if(!PressingAltfire() || JustReleased(BT_ALTATTACK)){
						return ResolveState("Zoomout_Sniper");
					}
		...
		ReadyGrenade:
			TNT1 A 0 A_JumpIf(!GetCvar("pb_toggle_aim_hold"), "ReadyGrenadeToggle");
			"H0F0" E 0;
			R0F6 E 1 {
		...
		ReadyGrenadeToggle:
			TNT1 A 0 A_JumpIfInventory("FiringUnderBarrelGrenadeLauncher", 1, 1);
			Goto ReallyReady;
			R0F6 E 1 {
				if (CountInv("DMRUpgraded") == 1) {A_SetWeaponSprite("H0F0");}
				if (CountInv("HDMRSniperMode") == 1) {A_SetWeaponSprite("H3F0");}
		...
		IdleLeft_Overlay:
			...
			P2SG B 1 {
				...
				if(CountInv("DualFiring")==0 || CountInv("RifleAmmo")<=0 || GetCvar("SingleDualFire")==1){
					if(PressingFire() || JustPressed(BT_ATTACK)){
						if(CountInv("HDMRSniperMode") == 1 && CountInv("LeftRifleAmmo") > 0) {
							return ResolveState("FireLeft_Sniper_Overlay");
						}
				...
				if(CountInv("DualWieldingDMRs") == 0) {
					return ResolveState("CancelOverlay");
				}
				return ResolveState(null);
			}
			Loop;
				...
				if(CountInv("DualFiring")==1 || CountInv("LeftRifleAmmo")<=0){
					if((PressingFire() || JustPressed(BT_ATTACK)) && GetCvar("SingleDualFire")==0){
						if(CountInv("HDMRSniperMode") == 1 && CountInv("RifleAmmo") > 0) {
							return ResolveState("FireRight_Sniper_Overlay");
						}
		...
		R2F4 A 1 BRIGHT {
			...
			if (CountInv("DMRUpgraded") == 1) {
				...
				A_PlaySoundEx("weapons/hdmr/fire", "Weapon");
...
;// 				PB_SpawnCasing("EmptyBrass",28,0,30,3,Frandom(5,8),Frandom(1,4));
			...
			if (CountInv("DMRUpgraded") == 1) {A_FireBullets (0.1, 0.1, -1, 36, "HitPuff", FBF_NORANDOM,8192,"Tracer", -4,-8);}
			else { A_FireBullets (0.1, 0.1, -1, 26, "HitPuff", FBF_NORANDOM,8192,"Tracer", -4,-8);}
			
			A_PB_ThrottledGunSmoke(-4, 0, 0, 'RifleAkimboSmokePhase');
			A_AlertMonsters();
			A_ZoomFactor(0.98);
		...
		H7F7 A 1 BRIGHT {
			A_ZoomFactor(0.98);
			A_PlaySoundEx("weapons/rifle/sniperfire", "Weapon");
			
			A_PB_RifleHeavyFireFX(1, 4, 0);
			A_AlertMonsters();
	...
	//FireRight_Sniper_Cooldown:
		...
		H7F6 A 1 BRIGHT {
			...
			A_FireCustomMissile("ShakeYourAssQuad", 0, 0, 0, 0);
			A_FireCustomMissile("YellowFlareSpawn",0,0,-4,0);
			A_Overlay(-42,"DualLeftMuzzleFlashHandler_Sniper");
            PB_WeaponRecoil(-6.4,1.2);
	...
	// ToDo: Update sprites based on HDMR mode
		...
		TNT1 A 0 A_Print("\ckNeed two \ctrifles \c-to dual-wield");
		Goto Ready3;
		SwitchToDualWield:
		"H6F7" "ABCDEF" 0;
		"H6F8" "ABCDEF" 0;
	...
	//This code will be reworked since the new equipment system breaks it
		...
		TNT1 A 0 {
			...
			A_ZoomFactor(1.0);
			A_PB_ThrottledGunSmoke(0, 0, 0, 'RifleAltSmokePhase');
        ...
        TNT1 A 0 A_JumpIfInventory("UnderBarrelGrenadeLoaded",1,"ReadyGrenade");
		...
		TNT1 A 0 {
			...
			A_PlaySoundEx("Weapons/GrenadeLoad", "Auto");
		}
		...
		TNT1 A 0 A_PlaySoundEx("LMGCLK", "Auto");
...
//Unupgraded Muzzle Flash handler
		...
		R0F8 A 1 BRIGHT {
			...
			A_PB_RifleAutoFireFX();
			A_Takeinventory("RifleAmmo",1);
			A_AlertMonsters();
			A_ZoomFactor(0.98);
			A_DoPBWeaponAction(WRF_ALLOWRELOAD|WRF_NOPRIMARY);
	...
	//LeftSniperFireCooldown:
		...
		"H4F8" B 1 BRIGHT;
		"H4F8" "CEGIK" 1 A_DoPBWeaponAction(WRF_ALLOWRELOAD|WRF_NOPRIMARY);
		"H4F8" K 1 Offset(0,34);
		"H4F8" K 2 Offset(0,36);
		"H4F8" K 3 Offset(0,38);
		"H4F8" K 1 Offset(0,34);
		"H4F8" "JJIHGFED" 1 A_DoPBWeaponAction(WRF_ALLOWRELOAD|WRF_NOPRIMARY);
		Goto Ready3;
	//Fire_Cooldown:
		...
		PrepareRifleGrenadeFire:
		TNT1 A 0 {
				if(CountInv("FiringUnderBarrelGrenadeLauncher") != 1) {
					PB_HandleCrosshair(89);
					A_GiveInventory("FiringUnderBarrelGrenadeLauncher", 1);
					A_StartSound("IronSights", 0);
					A_PlaySoundEx("weapons/sgl/inspect2", "Auto");
				}
				else {
					PB_HandleCrosshair(58);
					A_ZoomFactor(1.0);
        ...
        "####" "#" 0 {
			...
			if (CountInv("HDMRSniperMode") == 1 && CountInv("NewClip") >= 2) {
				return ResolveState("ContinueReloading");
			}
			else if (CountInv("HDMRSniperMode") == 0 && CountInv("NewClip") >= 1){
				return ResolveState("ContinueReloading");
			}
			return ResolveState(null);
		}
		...
		TNT1 A 0 {
 		...
 		TNT1 A 0 {
			if (CountInv("RifleAmmo") >= 31 && CountInv("HDMRSniperMode") != 1) {
				return ResolveState("Upgraded_SkipRightReload");
			}
		...
		"H96F" "LMNOPQR" 0;
 		TNT1 A 0 {
			if (CountInv("LeftRifleAmmo") >= 31 && CountInv("HDMRSniperMode") != 1) {
				return ResolveState("Upgraded_SkipLeftReload");
			}
		...
		"H96F" A 0 A_JumpIfInventory("HDMRSniperMode",1,2);
		"H8F6" A 0;
		"####" "ABCDEFG" 1;
		...
		TNT1 A 0 {
			if (CountInv("RifleAmmo") >= 31 && CountInv("HDMRSniperMode") != 1) {
				return ResolveState("Upgraded_InsertBullets5");
			}
		...
		TNT1 A 0 {
			if (CountInv("LeftRifleAmmo") >= 31 && CountInv("HDMRSniperMode") != 1) {
				return ResolveState("FinishInsertBullets");
			}
		...
		"R0F6" E 2;
		Stop;
		PDA_Preview_Rifle_Sniper:
		"H4F8" A 2 Bright;
		"H4F8" B 2 Bright;
		"H4F8" C 2;
		"H4F8" E 2;
		Stop;
		PDA_Preview_Rifle_ModeToSniper:
		"H2F0" "ABCDEF" 2;
		"H2F0" "GHI" 2;
		"H2F1" "AB" 2;
		"H2F3" "ABCD" 2;
		Stop;
		PDA_Preview_Rifle_Grenade:
		"H1F3" "AB" 2 Bright;
		"H1F3" "CDEF" 2;
		Stop;
		PDA_Preview_Rifle_Reload:
		"R2F0" "ABCDEF" 2;
		"R2F0" "MNOP" 2;
		"R2F0" "STUV" 2;
		"R2F0" "XYZ" 2;
