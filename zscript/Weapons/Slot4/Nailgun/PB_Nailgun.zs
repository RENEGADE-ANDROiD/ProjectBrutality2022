// PB_Nailgun - ZScript port (DECORATE PB_Weapon retired).

class PB_Nailgun : PB_WeaponBase
{
	default
	{
		//Title Nailgun
...
Weapon.SlotPriority 0.1;
	...
	//Nailgun
		...
		TNT1 A 0 {
			if (CountInv("NailgunHasUnloaded") == 1) { return ResolveState("ReadyToFireUnloaded"); }
			if (CountInv("PB_NailgunAmmo") == 0) { return ResolveState("ReadyToFireEmpty"); }
	...
	//NailGun Fire
        ...
        TNT1 A 0 A_FireBullets(0, 0, 1, 28, "shotpuff", 0, 250);
        ...
        TNT1 A 0 {
			...
			if (CountInv("NailgunHasUnloaded") == 1) { return ResolveState("ReloadNailgunUnloaded"); }
			if (CountInv("PB_NailgunAmmo") == 0) { return ResolveState("ReloadNailgunEmpty"); }
			if (CountInv("PB_NailgunAmmo") > 0) { return ResolveState("ReloadNailgun"); }
			return ResolveState(null);
		}
		ReloadNailgun:
		TNT1 A 0 {
        ...
        TNT1 A 0 {
			//Smart Check Inventory
			if (CountInv("NailgunHasUnloaded") == 1) { return ResolveState("ReloadJavelinContinuedFromUnloaded"); }
			if (((CountInv("PB_NailgunAmmo") < 20) && (CountInv("NewClip")) >= 20)) { return ResolveState("ReloadJavelinContinuedFromEmpty"); }
			if ((CountInv("PB_NailgunAmmo")+CountInv("NewClip")) >= 20) { return ResolveState("ReloadJavelinContinued"); }
			return ResolveState(null);
		}
        Goto NoAmmo;
		...
		PDA_Preview_Nail_Fire:
		"N2N0" A 2 Bright;
		"N2N0" B 2;
		"N2N0" "CDEF" 2;
		Stop;
		PDA_Preview_Nail_Reload:
		"N3N0" "ABCDEFG" 2;
		"N3N0" "HI" 2;
		"N3N1" "ABCDE" 2;
		"N3N2" "ABCDEF" 2;
