...
class PB_Carbine : PB_WeaponBase
{
	default
	{
		...
		//Category Weapons
		//Sprite CB00Z0
	// SpawnID (DECORATE editor only) 9450
	...
	ReadyToFire2:
		4A2G A 1 {
			if(CountInv("CarbineScope") == 1) {
				A_SetWeaponSprite("TNT1");
				A_Overlay(20,"ScopeBackground",true);
				A_OverlayPivotAlign(20,PSPA_CENTER,PSPA_CENTER);
				A_OverlayOffset(20,0,0);
				A_Overlayscale(20,1.0,1.0,WOF_INTERPOLATE);
				A_OverlayFlags(20,PSPF_RENDERSTYLE,true);
				A_OverlayRenderStyle(20,STYLE_Normal);
				A_Overlay(21,"ScopeReticle",true);
				A_OverlayPivotAlign(21,PSPA_CENTER,PSPA_CENTER);
				A_OverlayOffset(21,0,0);
				A_Overlayscale(21,1.0,1.0,WOF_INTERPOLATE);
				A_OverlayFlags(21,PSPF_RENDERSTYLE,true);
				A_OverlayRenderStyle(21,STYLE_Normal);
			} else {
				PB_CoolDownBarrel(0, 0, 5);
			}
		}
	...
	WheelCancel:
		TNT1 A 0 {
			...
			A_SetInventory("SelectCarbine_Sights",0);
	...
	ToggleScope:
		TNT1 A 0 {
			...
			if(CountInv("CarbineScope") == 0) {
				...
				A_Overlay(-19,"WheelCancel");
				A_Log("Optic switched: Scope");
			}
 			else {
				A_Setinventory("CarbineScope",0);
				A_Overlay(-19,"WheelCancel");
	...
	ZoomIn:
		...
		TNT1 A 0 {
			...
			A_OverlayOffset(6,0,0);
			A_Overlayscale(6,1.0,1.0);
			A_OverlayFlags(6,PSPF_RENDERSTYLE,true);
			A_OverlayRenderStyle(6,STYLE_Normal);
		}
	...
	ZoomInLower:
		...
		TNT1 A 0 {
			A_Overlay(21,"ScopeReticle");
			A_OverlayPivotAlign(21,PSPA_CENTER,PSPA_CENTER);
			A_OverlayOffset(21,0,0);
			A_Overlayscale(21,1.0,1.0);
			A_OverlayFlags(21,PSPF_RENDERSTYLE,true);
			A_OverlayRenderStyle(21,STYLE_Normal);
		}
		Goto Ready2;
