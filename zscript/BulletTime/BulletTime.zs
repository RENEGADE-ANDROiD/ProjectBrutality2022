class BulletTime : EventHandler
{
	...
	bool PB_BulletTimeAllowed()
	{
		if (multiplayer || netgame)
			return false;
		let cv = CVar.FindCVar("pb_bullettime");
		if (!cv || !cv.GetBool())
			return false;
	...
	**/
	...
	void initCvarVariables(bool fromBtActivation)
	{
		...
		let cvFreeze = CVar.FindCVar("pb_bullettime_allow_freeze");
		bool allowFreeze = cvFreeze && cvFreeze.GetBool();
		...
		if (!allowFreeze)
		{
			if (cvBtMultiplier < 1) cvBtMultiplier = 1;
			if (cvBtDodgeMultiplier < 1) cvBtDodgeMultiplier = 1;
			if (cvBtBerserkMultiplier < 1) cvBtBerserkMultiplier = 1;
			if (cvBtBerserkDodgeMultiplier < 1) cvBtBerserkDodgeMultiplier = 1;
		}
