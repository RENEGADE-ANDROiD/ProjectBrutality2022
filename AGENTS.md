...
zscript/                   Modern logic, included via ZSCRIPT.zc
  ...
  Weapons/
    BaseWeapon.zc
    PB_WeaponTacticalFeel.zc   PB tactical PSprite offsets + camera lean (pairs with Tilt++; see §4 Tilt++)
    FlamerStuff.zsc
...
- **Tilt++ (Nash) + PB tactical weapon feel (`pb_tac_*`).** Two features share the player camera roll and must stay in sync with DECORATE weapon bring-up (`SelectAnimation`, `SelectContinue`, `WeaponRespect`, `PB_RespectIfNeeded`, etc.):
  - **Files / handlers:** `zscript/TiltPlus/TiltPlusPlus.zc` (`Z_TiltMe`, given by `TiltPlusPlusHandler`), `zscript/TiltPlus/TiltPlusPlusMenu.zc`, `zscript/Weapons/PB_WeaponTacticalFeel.zc` (`PB_WeaponTilterInventory`, given by `PB_TacticalFeelHandler`). Both handlers are registered in `zmapinfo.txt` `AddEventHandlers`.
  - **`ZSCRIPT.zc` include order:** `#include "zscript/Weapons/PB_WeaponTacticalFeel.zc"` **must appear immediately before** `#include "zscript/TiltPlus/TiltPlusPlus.zc"` (and the menu shard) so `PB_WeaponTilterInventory` is defined when Tilt++ applies the merge.
  - **Camera roll:** PB tactical **lean** must **not** call `owner.A_SetRoll` by itself. `Z_TiltMe.Tilt_CalcViewRoll` merges Nash Tilt++ with **`PB_WeaponTilterInventory`**: `StripPriorLeanFromRoll`, `GetCameraLeanRollForMerge()`, `SetAppliedCameraLean` (tracks last tic’s merged lean so `(Owner.Roll + …) * 0.5` does not double-count). With **`pb_tac_enable` off**, use **`ResetCameraLeanMergeState()`** — do **not** zero **`Owner.Roll`** or Nash tilt is wiped until the next tic.
  - **Locks / wheel:** `Z_TiltMe.Tick` skips **`Tilt_CalcViewRoll`** while **`PB_LockScreenTilt`** is held (respect / scripted camera). **`GetCameraLeanRollForMerge()`** goes through **`ShouldBlockForSafety`** (includes **`PlayerWheelOpen`**, scoped ADS, glory/kick/equipment tokens, **`PB_LockScreenTilt`**, etc.).
  - **PSprite tactical offsets:** Movement lowering and strafe **rotation** apply only under **`holdBlend`**: **`(ready || reloading) && !blocked && !excluded`** where **`ready`** uses a **strict** **`IsReadyState()`** (explicit idle labels such as **`Ready`**, **`Ready3`**, **`ReadyToFire`**, **`Ready2`**, **`Ready4`**, **`Hold`**, ADS-ready variants — **not** a catch-all “unknown = ready”). **`Y`** lowering remains **`PSP_WEAPON`**-only. Strafe roll is **strip-and-reapplied** each tic on **`PSP_WEAPON`** plus overlay slots **`10`** and **`11`** when active (typical dual-wield hand layers); **`PSP_FLASH`** is left alone so **`FlashPunching`** is unaffected. Outside **`holdBlend`**, **`ClearAllTacticalRoll`** removes last tic’s additive rotation from those layers — **`ResetAppliedOffset`** clears tactical **`Y`** on the main weapon so raise/respect animations are not fought. Reload smoothing uses **`reloading`** (**`IsReloadState`**) alongside **`ready`** where needed.
  - **Per-weapon tactical tuning:** `ResolveTacticalWeaponProfile` in **`zscript/Weapons/PB_WeaponTacticalFeel.zc`** maps **`Weapon.GetClassName()`** to **`rollMul`**, **`loweringMul`** (movement-only), **`leanMul`**, **`pspDynamicYOffsetMul`** (scales movement lowering + step/reload dip — **not** wall smoothing; wall uses its own **`wallLoweringAmount`** so MG/minigun **`pspDynamicYOffsetMul` 0** still shows near-wall dip), **`pspEdgeHideMul`**, and **`syncDualOverlays`**. **`NO_TACTICAL_WEAPONS`** fully disables tactical for listed classes.
