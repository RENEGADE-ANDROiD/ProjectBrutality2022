...
- **Removed weapons (folded-out add-ons).** The following were stripped from the mod and **must not be re-added** as raw ports: **Hell Pistoler** (Slot 2), **HASG / "Lady Golide"** (Slot 3), **XM-21 Sniper** (Slot 4), **Old HMG** (Slot 4), **M41A Pulse Rifle** (Slot 4), **Chex Rifle** (Slot 4, was a WIP Chex Quest crossover easter egg), **Demon Tech Minigun** (Slot 9), **Pulse Cannon** + **Dual Pulse Cannons** (Slot 6), plus the Pulse Cannon upgrade, **Bio Acid Launcher** (Slot 8). Their `*.dec` files, sprite trees, sound trees, brightmap includes, GLDEFS light blocks, `DoomEdNum` entries, `pb_No*` cvars, menu rows, `language.enu` strings, `IsSelected` HUD blocks, `specialWheel_*` functions, ACS `ToggleWeapon*` / `TogglePulseCannonUpgrade` / `PBUpgradeChecker_PulseCannon` scripts, and friendly-marine variant (`Marine_PulseCannon*`) were all removed. `PB_762x51mm` (the shared rifle bullet used by `BDPBattleRifle.zs`) was relocated from the deleted `M41A.dec` into `actors/Weapons/Slot4/BattleRifleProjectiles.dec`. If you find a stale doc/sound/sprite/cvar reference to any of the names above, treat it as dead and remove it.
...
1. **Add-on layout:** Prefer an add-on folder that already contains top-level **`Sounds/`** and **`Sprites/`** (or equivalent) so all audio and graphics can be copied without hunting loose lumps. If upstream only ships a PK3, unpack to that shape before porting. Worked example: the **Lever Action** port (see `PORTING_ADDONS.md`) — `actors/Weapons/Slot2/LeverAction.dec`, `sounds/combat/weapons/LeverAction/`, `SPRITES/WEAPONS/Slot2/LeverAction/`, a `SNDINFO.txt` PBWeapons block, a `pb_No*` cvar + revolver spawner branch, a dedicated `IsSelected` block in `SBARINFO.txt`, and matching `PB_PICKUP_*` / `PB_WPNHINT_*` strings in `language.enu`. Newer add-on **`PB_Projectile` / `PB_HighCalMag`** types are mapped to `FastProjectile` + **`NewClip`** (see `PORTING_ADDONS.md`).
