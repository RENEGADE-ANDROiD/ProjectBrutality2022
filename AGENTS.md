...
- **DemonTechMinigun N-mode wheel pattern (reference for multi-mode weapons).** `DemonTechMinigun` (`actors/Weapons/Slot9/DTechMinigun.dec`) carries a 4-mode counter inventory `DemonTechMinigunMode` (max 3, values 0..3 = Plasma Volley / Shrink Beam / Acid Glob / Danmaku Spiral). The legacy 1-bit `DemonTechMinigunType` token is still defined and is **kept synced** to `Mode == 1` only, so older code paths and idle-sprite checks (`Ready3` / `Ready2` use `JumpIfInventory("DemonTechMinigunType",1,…)`, not `Mode`) keep working. `Fire` dispatches to dedicated branches by jumping on `Mode` (3→`FireDanmaku`, 2→`FireAcid`, 1→`Fire2`, fallthrough is mode 0). `AltFire` only toggles modes **0↔1** (the original load/unload animations); from mode 2 or 3 it routes through a silent `AltFireSnap` that resets `Mode` to 0 (taking up to 3 to clear) plus the type token. Mode picking from the wheel uses **four `Select_DTechMG_*` `Inventory` tokens** (`Plasma`, `Shrink`, `Acid`, `Danmaku`); `WeaponSpecial` checks them in order, takes all four, then takes/gives `Mode`/`Type` to land on the requested value. Wheel surface lives in `zscript/PbWheel/ev_core_special.zsc` as `void specialWheel_DTechMinigun()` (icons reuse `graphics/pywheel/Minigun_2.png` / `Minigun_1.png` / `grenade_acid.png` and `graphics/CSSG/SG_Danmaku.png`), and the dispatch case is `case 'DemonTechMinigun':` in the same file's `NetworkProcess` switch. New mode-3 projectile `DTechAcidGlob` bounces (`+DOOMBOUNCE`, `BounceCount 2`) and `A_SpawnItem("AcidSpot",…)` on death to lay a persistent green puddle (reusing the existing `AcidSpot` from `actors/Gore/GreenBlood.dec`); new mode-4 `DTechSpiralBolt : FastProjectile` rolls a translation (`DamRed` / `DamLightBlue` / `DamYellow` / `DamCyan` / `Green` from `trnslate.txt`) per shot for the rainbow spiral. **Recipe for adding more modes elsewhere:** (1) define a **counter** `Inventory` for the mode index plus one `Select_<Weapon>_<Mode>` token per choice; (2) keep any legacy 1-bit toggle synced to a specific mode value; (3) dispatch `Fire` by `JumpIfInventory("<Counter>", N, "FireMode<N>")` from highest to lowest; (4) gate `AltFire` to the modes whose original animations fit, snap-resetting from anywhere else; (5) add a `void specialWheel_<Weapon>()` and a matching `case '<ClassName>':` in `ev_core_special.zsc` `NetworkProcess`.
