...
- **Tilt++ (Nash) + PB tactical weapon feel (`pb_tac_*`).** Two features share the player camera roll and must stay in sync with DECORATE weapon bring-up (`SelectAnimation`, `SelectContinue`, `WeaponRespect`, `PB_RespectIfNeeded`, etc.):
  ...
  - **PSprite tactical offsets:** Movement lowering and strafe rotation on **`PSP_WEAPON`** apply only under **`holdBlend`**: **`(ready || reloading) && !blocked && !excluded`** where **`ready`** uses a **strict** **`IsReadyState()`** (explicit idle labels such as **`Ready`**, **`Ready3`**, **`ReadyToFire`**, **`Ready2`**, **`Ready4`**, **`Hold`**, ADS-ready variants — **not** a catch-all “unknown = ready”). Outside that, **`ResetAppliedOffset`** clears tactical **`Y`** so raise/respect animations are not fought (avoids first-select sprite jumps). Reload smoothing uses **`reloading`** (**`IsReloadState`**) alongside **`ready`** where needed.
