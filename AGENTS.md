...
- **Extensionless graphics still load.** Loose files with **no extension** (raw lumps on disk) are valid; GZDoom matches them by lump name. **`doomdefs.bm` / `BMAP/*.txt`** brightmap entries often use `map "Brightmaps/…/NAME"` **without** `.png` when the file is extensionless; **`SNDINFO`**, **`TEXTURES`**, and **`DECORATE`** likewise reference the **logical lump name**, not necessarily `NAME.png`. When checking whether an asset is present, search by **basename** (ripgrep, recursive directory listing without forcing `*.png`), not only extension-filtered globs.
