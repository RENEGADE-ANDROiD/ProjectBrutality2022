...
- **Tilt++ (Nash) + PB tactical weapon feel (`pb_tac_*`).** Two features share the player camera roll and must stay in sync with DECORATE weapon bring-up (`SelectAnimation`, `SelectContinue`, `WeaponRespect`, `PB_RespectIfNeeded`, etc.):
  ...
  - **Per-weapon tactical tuning:** `ResolveTacticalWeaponProfile` in **`zscript/Weapons/PB_WeaponTacticalFeel.zc`** maps **`Weapon.GetClassName()`** to **`rollMul`**, **`loweringMul`** (movement-only lowering before **`cvWallLowering`** is applied — so **`loweringMul` 0** does not delete wall-aware weapon dip), **`leanMul`**, **`pspDynamicYOffsetMul`**, **`pspEdgeHideMul`**, and **`syncDualOverlays`**. **`NO_TACTICAL_WEAPONS`** fully disables tactical for listed classes.
