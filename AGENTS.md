...
- **Other shipped weapon-special wheels.** `LeverAction` (**`Select_LeverAction_357` / `Select_LeverAction_Marlin`**, caliber swap); **`PB_M1Plasma`** (**`Select_M1Plasma_Single` / `Select_M1Plasma_Dual`**, dual-wield gate); **`PB_PulseCannon`** (**`Select_PulseCannon_Gravity` / `Select_PulseCannon_Super`**, `PulseCannonMod` alt-fire mode). Each adds **`case '<ClassName>':`** → **`specialWheel_*()`** in `zscript/PbWheel/ev_core_special.zsc`; **`WeaponSpecial`** branches these **`Select_*`** tokens before the legacy tap-to-cycle behavior.
